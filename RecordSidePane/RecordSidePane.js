var Ben = Ben || {};
Ben.JS = Ben.JS || {};
Ben.JS.RecordSidePane = Ben.JS.RecordSidePane || {
    ctx:
    {
        ...
        script: {        
            ...
            namespace: "Ben.JS.RecordSidePane",
            indentCount: 0,
            verbose: true
        },
    ...
    onRecordSelect: async function (context, configPaneWidth) {
        ...
        try {
            ...
            await _._openRecordPane(context.getFormContext(), configPaneWidth.replace("px",""));
        }
        catch(err) {
            _._trace.error(fncName, err);
        }
    ...
    _openRecordPane: async function (formContext, configPaneWidth) {
        // Private-Function - It opens the Preview Pane and creates a new one if there is no existing pane.
        let 
            fncName = "_openRecordPane",
            _ = Ben.JS.RecordSidePane;

        try {
            _._trace.enterScope(fncName);
       
            if (!parent.Xrm.App.sidePanes.getPane("RecordSidePane")) {
                ...
                RecordSidePane = await parent.Xrm.App.sidePanes.createPane(selectedRecordPane);
            }     
            ...
            await RecordSidePane.navigate(selectedRecordPageInput);
    ...
    _closeRecordPane: async function(){
        // Private-Function - It closes the Preview Pane.
        let 
            fncName = "_closeRecordPane",
            _ = Ben.JS.RecordSidePane

        try {
            _._trace.enterScope(fncName);
            if (parent.Xrm.App.sidePanes.getPane("RecordSidePane")) {
                if(parent.Xrm.App.sidePanes.getPane("RecordSidePane") == Xrm.App.sidePanes.getSelectedPane()) {
                    _._trace.log("Pane is selected, collapse it now.")
                    parent.Xrm.App.sidePanes.state = 0;
                }
                _._trace.log("Closing pane now.")
                await parent.Xrm.App.sidePanes.getPane("RecordSidePane").close();
            }
            else {
